...
def comment_news():
    ...
    # 添加到数据库
    # 为什么要自己commit(), 因为在return的时候需要用到comment的id
    try:
...
def collect_news():
    ...
    if action == "cancel_collect":
        ...
        if news in user.collection_news:
            user.collection_news.remove(news) # user.collection_news是一个收藏的新闻列表,在就移除
        else:
            # 收藏
...
def news_detail(news_id):
    ...
    # 更新新闻的点击数量
    # 此处不用再去写db.session.commit操作 因为在配置中已经SQLALCHEMY_COMMIT_ON_TEARDOWN = True
    news.clicks += 1
